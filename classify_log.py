...
def classify_log(doc):
    ...
    if isinstance(doc, dict) and 'fields' in doc:
        ...
        fields_map = doc.get('fields', {}) or {}

        # If event.original exists, parse raw JSON and merge flattened fields into it
        if 'event.original' in fields_map:
            try:
                raw_log_str = fields_map['event.original'][0]
                parsed = json.loads(raw_log_str)
                # keep original metadata (like _index)
                if original_index:
                    parsed['_index'] = original_index
                # Merge flattened fields into parsed (only if key not present)
                ...
                pass  # Fall back to original doc with fields
        else:
            # No event.original — flatten fields onto the doc for dotted-key access
            for k, v in fields_map.items():
                if isinstance(v, list) and len(v) > 0:
                    if k not in doc:
                        doc[k] = v[0]
